...
function loop() { 
  if (game.on === true) {
    ...
    game.cycles.forEach(function(cycle) {
      ...
      if (gameLevel === 3) {
        console.log("game.disks " + game.disks);
        game.disks.forEach(function(disk) {
          console.log("disk " + disk)
          disk.makeDisk().eraseDisk(ctx).moveDisk().drawDisk(ctx);
        });
      }
...
function setupLevel() {
  ...
  if (gameLevel !== 1) {
    ...
    coordArray.forEach((coordPair) => {
      drawObstacles(coordPair.x, coordPair.y);
      game.createObstacles(coordPair.x, coordPair.y);
    });
  }
  game.on = !game.on;
  startGame();
...
function end(color) {
  ...
  if (gameLevel === 1) {
    ...
    gameText[2].innerText = 'LOADING... Please wait';
    loading();
  } else if (gameLevel === 2) {
    gameText[0].innerText = 'Round ' + gameLevel + 
